...
def create():
    ...
    if request.method == 'POST':
        ...
        plant_insert_id = plants.insert_one(new_plant).inserted_id

        return redirect(url_for('detail', plant_id=plant_insert_id))
...
def detail(plant_id):
    ...
    # collection.
    find_harvests = harvests.find()

    context = {
        'plant' : plant_to_show,
        'harvests': find_harvests
    }
...
def delete(plant_id):
    ...
    # the given plant id.
    harvest_delete = harvests.find({'plant_id': ObjectId(plant_id)})
    for item in harvest_delete: 
